import { gameScale } from './game.js';
import { globalScaleFactor } from './main.js';
...
export class BackgroundCache {
  constructor(name) {
    ...
    //this.c = offscreen.getContext('2d');
  ...
  draw(c, xmin, ymin, xmax, ymax) {
    let ratio = globalScaleFactor(); // Retina support
    ...
    if (this.modificationCount != gameState.modificationCount || xmin < this.xmin || xmax > this.xmax || ymin < this.ymin || ymax > this.ymax || this.ratio != ratio) {
      ...
      let viewportWidth = 2 * (xmax - xmin);
      let viewportHeight = 2 * (ymax - ymin);
      ...
      let width = Math.ceil(viewportWidth * gameScale);
      let height = Math.ceil(viewportHeight * gameScale);
      this.width = width;
      this.height = height;
