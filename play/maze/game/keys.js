import { keyCodeArray } from '../util/keyCode-array.js';
...
export let Keys = {
  ...
  keyCodeHTML: function(keyCode) {
    let name = keyCodeArray[keyCode] || '&iquest;';
    let html = toTitleCase(name).replace(' ', '<br>');
  ...
  load: function() {
    for (let name in this.keyMap) {
      let keyCode = parseInt(getLocalStorage(name), 10);
      if (!isNaN(keyCode)) {
