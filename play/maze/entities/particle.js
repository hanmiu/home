...
class ParticleInstance {
  ...
  draw(c) {
    ...
    case PARTICLE_TRIANGLE:
      let v1 = this.m_position.add(this.m_velocity.mul(0.04));
      let v2 = this.m_position.sub(this.m_velocity.flip().mul(0.01));
      let v3 = this.m_position.add(this.m_velocity.flip().mul(0.01));
      c.fillStyle = cssRGBA(this.m_red, this.m_green, this.m_blue, this.m_alpha);
      c.beginPath();
      c.moveTo(v1.x, v1.y);
      c.lineTo(v2.x, v2.y);
    ...
    case PARTICLE_LINE:
      let dx = Math.cos(this.m_angle) * this.m_radius;
      let dy = Math.sin(this.m_angle) * this.m_radius;
...
export class Particle {
  ...
  static take() {
    let particle = (count < maxCount) ? particles[count++] : particles[maxCount - 1];
    particle.init();
    return particle;  
  }
  ...
  static tick(seconds) {
    for(let i = 0; i < count; i++) {
      let isAlive = particles[i].tick(seconds);
      if (!isAlive) {
  ...
  static draw(c) {
    for(let i = 0; i < count; i++) {
      ...
      let pos = particle.m_position;
      if (pos.x >= gameState.drawMinX && pos.y >= gameState.drawMinY && pos.x <= gameState.drawMaxX && pos.y <= gameState.drawMaxY) {
        particle.draw(c);
      }
    }
  }
