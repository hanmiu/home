function add_hakjeok_row(el_page, n, date, content, note) {
    let tbody = el_page.querySelector('.학적사항-table tbody');
    let tr = document.createElement('tr');
    tr.className = '학적-row';
    tr.setAttribute('data-hakjeok-n', n);
    tr.innerHTML = `
        <td contenteditable="true" spellcheck="false" class="학적날짜${n}">${date || ''}</td>
        <td contenteditable="true" spellcheck="false" class="학적내용${n}" style="text-align: right;">${content || ''}</td>
        <td contenteditable="true" spellcheck="false" class="학적특기${n}">${note || ''}</td>
        <td class="학적-action-cell"><button class="remove-hakjeok" title="이 행 삭제">×</button></td>
    `;
    tbody.appendChild(tr);
    return tr;
}

function renumber_hakjeok_rows(el_page) {
    let rows = el_page.querySelectorAll('.학적사항-table tbody tr.학적-row');
    rows.forEach((tr, idx) => {
        let n = idx + 5; // extra rows start at 5
        tr.setAttribute('data-hakjeok-n', n);
        let tds = tr.querySelectorAll('td');
        tds[0].className = '학적날짜' + n;
        tds[1].className = '학적내용' + n;
        tds[2].className = '학적특기' + n;
    });
}

function get_hakjeok_count(el_page) {
    let base = 4;
    let extra = el_page.querySelectorAll('.학적사항-table tbody tr.학적-row').length;
    return base + extra;
}
...
<h2>유치원생활기록부</h2>
<div class="control">
    <button class="go-up" title="위로 이동">⤒</button>
    <button class="go-down" title="아래로 이동">⤓</button>
...
<div class="section-3">
    ...
    <table class="학적사항-table">
        <thead>
            <tr>
                <td>연.월.일. | 구분</td>
                <td style="width: 400px;">내용</td>
                <td>특기사항</td>
                <td class="학적-action-cell" style="width: 32px;"></td>
            </tr>
        </thead>
        <tbody>
            <tr class="학적-base-row">
    ...
    <div class="학적-add-wrap">
        <button class="add-hakjeok">+ 학적 행 추가</button>
    </div>
</div>
<div class="section-4">
    <h3>4. 출결상황 
...
<div class="section-7">
    ...
    if(cols.length === infos.length) {
        for(let i = 0; i < cols.length; i++) {
            let query = cols[i];
            if(query) {
                // Handle dynamic 학적 rows beyond 4
                if(query.match(/^학적(날짜|내용|특기)\d+$/) && infos[i].trim()) {
                    let num = parseInt(query.match(/\d+$/)[0]);
                    if(num > 4) {
                        // Ensure the row exists
                        let existing = el_page.querySelector('.' + query);
                        if(!existing) {
                            add_hakjeok_row(el_page, num, '', '', '');
                        }
                    }
                }
